...
# class WGB defines:
#    xyz(building_coordinates) -> returns { x: y: z: } in meters
...
class IFM(object):
    def __init__(self):
        ...
        self.origin_gps = { "acp_lat": 52.20977,
                            "acp_lng": 0.087,
    ...
    def xyz(self, building_coordinates):
    ...
    def gps(self, building_coordinates):
        xyz = self.xyz(building_coordinates)
        # rotate for x,y aligned to true North
        x = xyz["x"] * self.rotate_matrix[0][0] + xyz["y"] * self.rotate_matrix[0][1]
        y = xyz["x"] * self.rotate_matrix[1][0] + xyz["y"] * self.rotate_matrix[1][1]
        ...
        alt = self.origin_gps["acp_alt"] + xyz["z"] # altitude meters (+ve = Up)
